import type { EventOptions, InternalEventOptions } from '../../abstract-control';
...
export class FormGroup<C extends Controls = Controls> extends AbstractControl<FormGroupValueType<C>> {
  ...
  public setValue(value: DeepPartial<FormGroupValueType<C>>, options: EventOptions = {}): void {
    Object.entries(value).forEach(([name, controlValue]) => {
      const control = this.controls[name];

      if (control) {
        control.setValue(controlValue, { emitEvent: options.emitEvent, onlySelf: true });
      }

      console.warn(
        `Cannot set value of the control with name "${name.toString()}" because it does not exist in the form group.`,
      );
    });
